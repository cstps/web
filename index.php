...
$view_news = "";
$sql = "select * FROM `news` WHERE `defunct`!='Y' AND `title`!='faqs.$OJ_LANG' "
     . "ORDER BY `importance` ASC,`time` DESC LIMIT 50";
...
foreach ($result as $row) array_push($chart_data_all, array($row['md'], $row['c']));
...
$result = mysql_query_cache($sql);
$chart_data_ac = array();
foreach ($result as $row) array_push($chart_data_ac, array($row['md'], $row['c']));
...
if (isset($_SESSION[$OJ_NAME.'_'.'administrator'])) {
    ...
    $result = mysql_query_cache($sql);
    $speed = (($result[0][0] ?? 0) ? $result[0][0] : 0).'/min';
} else {
...
/* ★삭제됨: 자동 XHR + location.reload() 스크립트
if (isset($OJ_LONG_LOGIN) && $OJ_LONG_LOGIN && (!isset($_SESSION[$OJ_NAME.'_user_id'])) && isset($_COOKIE[$OJ_NAME."_user"]) && isset($_COOKIE[$OJ_NAME."_check"])) {
