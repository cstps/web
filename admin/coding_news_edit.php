...
if(isset($_POST['news_id'])){
  ...
  $sql = "UPDATE `coding_news` SET `title`=?,`time`=now(),`content`=?,user_id=? WHERE `news_id`=?";
  //echo $sql;
  pdo_query($sql,$title,$content,$user_id,$news_id) ;

  header("location:coding_news_list.php");
  exit();
}else{
  $news_id = intval($_GET['id']);
  $sql = "SELECT * FROM `coding_news` WHERE `news_id`=?";
...
?>
  ...
  <form method=POST action=coding_news_edit.php>
