...
function addFileToZip($path, $zip) {
	...
	$handler = opendir($path); //지정된 path위치에 폴더를 열고
	/*
	주기적으로 폴더 아래의 모든 파일 및 폴더 읽기
	그 중에 $filename = readdir($handler)是每次循环的时候将读取的文件名赋值给$filename，
...
<button onclick="phpfm(0)" >백업파일보기</button>
